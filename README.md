...
支持网易云音乐、QQ 音乐、酷狗、酷我、Spotify、汽水音乐、咪咕音乐、小米音乐、华为音乐、Apple Music、YouTube Music、Amazon Music，以及其他正确发布系统媒体元数据的播放器。网易云、QQ 音乐、酷狗、酷我和汽水会优先使用各自乐库；汽水可直接使用 MediaSession 曲目 ID 获取原生逐字歌词与中文翻译，ID 不可用时按歌名、歌手和时长搜索，随后再依次查询其余乐库。Android 4.4 优先使用旧式 `RemoteControlClient`，没有有效媒体曲目时尝试通知识别，具体通知格式仍需实机验证。
...
- Android 5.0 以上优先读取 `MediaSession`，Android 4.4 优先读取 `RemoteControlClient`。媒体接口没有有效歌曲时，尝试解析音乐通知；最低系统版本仍为 Android 4.4。
- 通知兜底支持标准标题/正文/副标题/大文本/多行文本，以及能正常展开的自定义通知文字。已知音乐软件的常驻通知或带播放控制的通知才参与识别；不同软件版本的布局仍需实机验证，不能保证所有主流播放器都可识别。此路径不新增 AIDL。
- 通知中的歌名、歌手可以用于匹配歌词；通知发布时间不是播放进度，未提供进度时不会从零伪造自动滚动，可使用歌词浏览功能。播放器已有正常媒体信息时，通知不会覆盖它。
- U 盘/本地歌曲可以通过播放器通知识别，联网匹配沿用路径、曲序、扩展名和音质标记清理规则。开启本地歌词后，播放器提供可访问的完整文件路径时会优先读取同目录同名 `.lrc`；Android 5.0+ 也可授权歌词目录。只有文件名时无法推断 U 盘挂载路径，通知也没有歌曲信息时无法自动识别。
- 比亚迪 DiLink 4.0 系统媒体中心（`com.byd.mediacenter`）目前无法适配：它播放本地文件时不提供歌曲元数据、标准媒体通知或可供普通应用读取的公开接口，且控制服务受签名/特权权限保护。请使用会输出完整媒体信息的播放器（例如已验证可识别的 Salt Player）。
- 手动重新匹配跳过歌曲匹配缓存；各乐库原有歌曲 ID 缓存仍可复用。缓存按乐库、歌曲信息及已知时长区分，清除应用数据/卸载会删除缓存；不同元数据或版本可能需要重新匹配。
